...
from common.mappers import dataframe_to_raw_records, dataframe_to_target_records, normalize_ct_columns
...
@pytest.mark.parametrize("load_ct_dataframe, load_expected_data", [
    ("./calculation_tool/data/test-ADA-1.csv", "./calculation_tool/data/test-ADA-1-exp.csv")
    ], indirect=["load_ct_dataframe", "load_expected_data"])
def test_csv_data1(load_ct_dataframe: pd.DataFrame, load_expected_data: pd.DataFrame, mock_config: ConfigProtocol):
